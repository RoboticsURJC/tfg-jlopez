...
int main(int argc, char **argv)
{
  ...
  // Recuperar parámetros del servidor de parámetros de ROS
  n.getParam("left_wheel_name", robot_cfg.left_wheel_name);
  n.getParam("right_wheel_name", robot_cfg.right_wheel_name);
  n.getParam("enc_counts_per_rev", robot_cfg.enc_counts_per_rev);
  n.getParam("robot_loop_rate", robot_cfg.loop_rate);
  n.getParam("left_motor_pin", robot_cfg.left_motor_pin);
  n.getParam("right_motor_pin", robot_cfg.right_motor_pin);

  // Crear la instancia del robot con la configuración proporcionada
  DiffDriveArduino robot(robot_cfg);
  controller_manager::ControllerManager cm(&robot);
  ...
  // Configurar la tasa de bucle principal
  ros::Rate loop_rate(robot_cfg.loop_rate);

...
package_name = 'pibotj_rrv2'
launch_folder_name = 'pibotj_rrv2/launch'
...
setup(
    ...
    entry_points={
        'console_scripts': [
            ...
            'motors_controller_node = pibotj_rrv2.motors_controller_node:main',
            
            # nodo que obtiene los valores de latitud y longitud del puerto serie
            'gps_node = pibotj_rrv2.gps_node:main',

            # nodo que publica la cámara correctamente
            'camera_node = pibotj_rrv2.camera:main',

            # nodo que publica cámara + tflite            
            'camera_tfv1_node = pibotj_rrv2.camera_tfv1_node:main',

            # nodo que publica cámara + tflite + Coral           
            'camera_tfv2_node = pibotj_rrv2.camera_tfv2_node:main',

            # nodo que publica cámara + tflite + Coral en funcionamiento y detecta bache en función de la máscara
            # publica un array con las coordenadas detectadas de la máscara  cada 3s 
            'camera_tfv3_node = pibotj_rrv2.camera_tfv3_node:main',

            # nodo que publica cámara + tflite + Coral en funcionamiento y detecta bache en función de la máscara
            # publica un array con las coordenadas detectadas de la máscara cada 0.5s
            'camera_tfv4_node = pibotj_rrv2.camera_tfv4_node:main',

            # nodo que te dice en qué lado se sitúa el robot en función de la detección de las líneas
            'camera_dl_node = pibotj_rrv2.camera_dl_node:main',

            # nodo que ejecuta vff
            'camera_vff_node = pibotj_rrv2.camera_vff_node:main',

            # nodo que publica cámara + modelo pin hole (usando post-it)          
            'camera_pinhole_node = pibotj_rrv2.camera_pinhole_node:main',

            # nodo que publica cámara + modelo pin Hole + área funcionando + min coords
            'camera_pinholev2_node = pibotj_rrv2.camera_pinholev2_node:main',

            # nodo que publica cámara + modelo pin Hole + área funcionando
            'camera_pinhole_web_node = pibotj_rrv2.camera_pinhole_web_node:main',

            # nodo que publica cámara + modelo pin Hole + min coords
            'camera_pinhole_vff_node = pibotj_rrv2.camera_pinhole_vff_node:main',

            # nodo controlador para controlar los motores por la interfaz web
            'motors_controller_web_node = pibotj_rrv2.motors_controller_web_node:main',

            # nodo controlador para controlar los motores en el entorno controlado (fórnula lenta)
            'motors_vff_node = pibotj_rrv2.motors_vff_node:main',

            # nodo controlador para controlar los motores en el entorno controlado (fórmula rápida)
            'motors_vffv2_node = pibotj_rrv2.motors_vffv2_node:main'
